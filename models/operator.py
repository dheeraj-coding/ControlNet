...
class Adaptive_Routing(torch.nn.Module):
    def __init__(self, n_res, dim, text_dim, temperature=1.):
        ...
        self.mlp_2 = networks.MLP(text_dim, self.get_num_adain_params(self.res_2), text_dim, 3, norm='none',
                                  activ='relu')
        self.res2_2 = networks.ResBlocks(n_res, dim, 'adain', 'relu', pad_type='reflect')
        self.mlp2_2 = networks.MLP(text_dim, self.get_num_adain_params(self.res2_2), text_dim, 3, norm='none',
                                   activ='relu')
        self.res3_2 = networks.ResBlocks(n_res, dim, 'adain', 'relu', pad_type='reflect')
        self.mlp3_2 = networks.MLP(text_dim, self.get_num_adain_params(self.res3_2), text_dim, 3, norm='none',
                                   activ='relu')
